...
import pytest
from agent_tool_budget import BudgetExhausted, ToolBudget, ToolUsage, make_tool_budget

def test_use_within_budget():
    b = ToolBudget({"search": 3})
    b.use("search"); b.use("search"); b.use("search")
    assert b.used("search") == 3

def test_use_exhausts_raises():
    b = ToolBudget({"search": 2})
    b.use("search"); b.use("search")
...
def test_exception_attributes():
    b = ToolBudget({"tool": 2})
    b.use("tool"); b.use("tool")
    try:
        b.use("tool"); assert False
...
def test_exhausted_false():
    b = ToolBudget({"t": 3}); b.use("t")
    assert b.exhausted("t") is False

def test_exhausted_true():
    b = ToolBudget({"t": 2}); b.use("t"); b.use("t")
...
def test_explicit_overrides_wildcard():
    ...
    for _ in range(10): b.use("search")
